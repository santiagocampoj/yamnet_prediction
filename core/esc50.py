...
original source are always in the same fold. We preserve this
structure so train/val/test splits never leak frames.
...
ESC50_ZIP   = "./datasets/esc50.zip"
ESC50_ROOT  = "./datasets/ESC-50-master"
ESC50_CSV   = "./datasets/ESC-50-master/meta/esc50.csv"
ESC50_AUDIO = "./datasets/ESC-50-master/audio/"
...
def download(progress_callback=None) -> bool:
    ...
    try:
        urllib.request.urlretrieve(ESC50_URL, ESC50_ZIP, reporthook=reporthook)
...
def prepare_train_folder(selected_classes: list[str],
    ...
    """
    Copy audio files for selected_classes into out_dir/class_name/*.wav
    """
    df = get_metadata()
...
def load_dataset_with_folds(selected_classes: list[str],
    ...
    """
    Load ESC-50 audio files for selected classes, preserving fold information.

    This is the correct way to use ESC-50 — folds must be passed to
    classifier.train() so the clip-level split respects the original
    fold structure.

    Returns
    -------
    waveforms : list of np.ndarray   mono float32 at sr
    labels    : list of str          class names
    folds     : list of int          fold number (1-5) per clip
    """
    # from core.yamnet import load_audio_file
